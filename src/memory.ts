...
// THIS FILE IS BASED ON THE MODEL CONTEXT PROTOCOL SERVERS MEMORY SERVER
// CHECK THESE LINKS FOR THE ORIGINAL SOURCE AND DOCUMENTATION:
...
export class MemoryTools {
  ...
  constructor(private server: McpServer) {
    this.knowledgeGraphManager = undefined as any;
  ...
  private async initializeMemory() {
    ...
    this.knowledgeGraphManager = new KnowledgeGraphManager(MEMORY_FILE_PATH);
  }
  ...
  private async registerTools(server: McpServer) {
    ...
    server.registerTool(
      ...
      async ({ entities }) => {
        const result = await this.knowledgeGraphManager.createEntities(
          entities
        );
        return {
          content: [
    ...
    server.registerTool(
      ...
      async ({ relations }) => {
        const result = await this.knowledgeGraphManager.createRelations(
          relations
        );
        return {
          content: [
    ...
    server.registerTool(
      ...
      async ({ observations }) => {
        const result = await this.knowledgeGraphManager.addObservations(
          observations
        );
        return {
          content: [
    ...
    server.registerTool(
      ...
      async ({ query }) => {
        const graph = await this.knowledgeGraphManager.searchNodes(query);
        return {
          content: [
            { type: "text" as const, text: JSON.stringify(graph, null, 2) },
          ],
    ...
    server.registerTool(
      ...
      async ({ names }) => {
        const graph = await this.knowledgeGraphManager.openNodes(names);
        return {
          content: [
            { type: "text" as const, text: JSON.stringify(graph, null, 2) },
          ],
