...
export class WttrInTools {
  ...
  private async registerTools(server: McpServer) {
    server.tool(
      ...
      async ({ city }) => {
        const weatherData = await this.makeRequest<WeatherResponse>(city);
        ...
        const report = this.formatWeatherReport(weatherData, city);
  ...
  async makeRequest<T>(city: string): Promise<T | null> {
    try {
      const response = await fetch(
        `${SERVICE_API}/${encodeURIComponent(city)}?format=j1`,
        {
          headers: {
  ...
  private formatWeatherReport(data: WeatherResponse, city: string): string {
